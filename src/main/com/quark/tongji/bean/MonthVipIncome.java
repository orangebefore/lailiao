 ...
 *
 */
public class MonthVipIncome extends ChartTable {

	public MonthVipIncome(String req) {
		super(req);
		String month = req;
		...
		for (TongjiCharge tongjiRegist : regist) {
			...
			for (int i = 1; i < days+1; i++) {
				...
				TongjiCharge regist_model = TongjiCharge.dao.findFirst(
						"select sum(money) as money from tongji_charge where catalog=? and charge_date=?", catalog,
						month + "-" + day);
		...
		this.setChart_title(month);
		this.setChart_sub_title("总数：" + total.toString());
		this.setTitle(month + " 收入统计 - 共：" + total.toString());
	}
}
