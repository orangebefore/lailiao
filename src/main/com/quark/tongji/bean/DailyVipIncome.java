 ...
 *
 */
public class DailyVipIncome extends ChartTable {

	{
	
	}
	public DailyVipIncome(String req) {
		super(req);
		...
		for (TongjiCharge tongjiRegist : regist) {
			...
			for (int i = 1; i < 25; i++) {
				...
				TongjiCharge regist_model = TongjiCharge.dao.findFirst(
						"select sum(money) as money from tongji_charge where catalog=? and regist_hour=?", catalog,
						request_date + " " + hour);
		...
		this.setTitle(request_date + " 收入统计 - 共：" + total.toString());
	}
}
