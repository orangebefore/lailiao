...
public class FrozenUsers extends Controller {
	...
	public void userInfo(){
		...
		setAttr("city", city);
		render("/admin/FrozenUserInfo.html");
	}
	/**
	 * 1:甜蜜度,2:热度
	 */
	...
	public void report() {
		...
		blPage = BlackList.dao.paginate(currentPage, PAGE_SIZE,
				"select * ",
		...
		for(BlackList user:blPage.getList()){
			...
			int black_user_id = user.get("black_user_id");
			User user3 = User.dao.findById(black_user_id);
			int status = user3.get("status");
			String nickname2 = user3.getStr("nickname");
			if (nickname2.equals("")) {
				nickname2 = user3.getStr("telephone");
			}
			user.put("status", status);
			user.put("black_nickname", nickname2);
			List<BlackList> blackLists = BlackList.dao.find("select count(black_user_id) from black_list where black_user_id="+black_user_id);
			user.put("report_size", blackLists.size());
		}
		setAttr("list", blPage);
		setAttr("pn", currentPage);
	...
	public void unfreeze() throws Exception{
		...
		user.set(user.status, status).set(user.black_status, black_status).update();
		RongToken.unBlockUser(""+black_user_id, FormatType.json);
		redirect("/admin/FrozenUsers/list?pn="+currentPage+"&is_vip="+is_vip+"&sex="+sex+"&province="+URLEncoder.encode(province, "UTF-8")+"&city="+URLEncoder.encode(city, "UTF-8")+"&kw="+URLEncoder.encode(kw, "UTF-8"));
	}
	...
	public void free() throws Exception{
		...
		redirect("/admin/FrozenUsers/report?pn="+currentPage);
	}
	public void add() {
	...
	public void addCommit() {
		...
		boolean save = user.set(User.telephone, telephone).set(User.password, MD5Util.string2MD5(password))
				...
				.set(User.province, SelProvince).set(User.city, SelCity)
				.set(User.is_vip, 1).set(User.vip_from_datetime, "2016-02-23 18:39:53")
				.set(User.heart, heart).set(User.vip_end_datetime, "2016-07-23 18:39:53")
				.set(User.income, income).set(User.edu, edu).set(User.shape, user_shape)
				...
				.set(User.regist_hour, DateUtils.getCurrentDateHours())
				.set(User.hot, 10).set(User.sweet, 10)
				.save();
		if (save) {
			int user_id = user.get("user_id");
			...
			if (user_tags!=null) {
				for (int i = 0; i < user_tags.length; i++) {
					...
					userTag.set(userTag.user_id, user_id).set(userTag.tag, user_tags[i])
					.set(userTag.status, 1).set(userTag.post_time, DateUtils.getCurrentDateTime())
					.set(userTag.type, sex).save();
				}
			}
		} 
		redirect("/admin/FrozenUsers/list");
	}
}
