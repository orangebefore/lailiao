 ...
 */
@Before(Login.class)
public class MenUsers extends Controller {
	public void delete() {
		int user_id = getParaToInt("user_id");
		User.dao.deleteById(user_id);
		redirect("/admin/MenUsers/list");
	}
	...
	public void list() throws ParseException {
		...
		String filter_sql = " status=1 and sex=1 ";
		if (is_vip != 2) {
	...
	public void userInfo() {
		...
		List<Price> price = Price.dao.find("select * from price order by price asc,post_time desc");
		setAttr("price", price);

		render("/admin/MenUserInfo.html");
	}
	...
	public void unfreeze() throws Exception {
		...
		// 设置会员
		int vip = getParaToInt("vip");
		if (vip != 0) {
			Price price = Price.dao.findFirst("select * from price where price_id=?",vip);
			int days = price.get(Price.days);
			Timestamp end = user.getTimestamp("vip_end_datetime");
			System.out.println(days);
			System.out.println(end);
			String end_time = DateUtils.getAddDaysString2(days, end.toString());
			user.set(User.is_vip, 1).set(User.vip_end_datetime, end_time).update();
		}
		...
		redirect("/admin/MenUsers/list?pn=" + currentPage + "&is_vip=" + is_vip + "&sex=" + sex + "&province="
	...
	public void free() {
		...
		redirect("/admin/MenUsers/report?pn=" + currentPage);
	}
	...
	public void addCommit() {
		...
		boolean save = user.set(User.telephone, telephone).set(User.password, MD5Util.string2MD5(password))
				...
				.set(User.is_vip, 1).set(User.vip_from_datetime, "2016-02-23 18:39:53").set(User.heart, heart)
				.set(User.vip_end_datetime, "2016-07-23 18:39:53").set(User.income, income).set(User.edu, edu)
				.set(User.shape, user_shape).set(User.marry, marry).set(User.drink, drink).set(User.smoke, smoke)
