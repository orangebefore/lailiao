...
final class ActionMapping {
	 ...
	 * log4j
	 */
    static Logger log = Logger.getLogger(ActionMapping.class.getClass());
	...
	private Set<String> buildExcludedMethodName() {
		...
		for (Method m : methods) {
			//if (m.getParameterTypes().length == 0)//move by kingsley to user multiple args.
	...
	void buildActionMapping() {
		...
		for (Entry<String, Class<? extends Controller>> entry : routes.getEntrySet()) {
			...
			for (Method method : methods) {
				...
				if (!excludedMethodName.contains(methodName)){// && method.getParameterTypes().length == 0) {//move by kingsley to user multiple args.
